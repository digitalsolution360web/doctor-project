                ...
                {sidebarCategories.map((cat, idx) => (
                  <li key={idx}>
                    ...
                    >
                      ...
                      {cat.count > 0 && (
                        <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${cat.active ? 'bg-teal-100 text-teal-700' : 'bg-slate-100 text-slate-400'
                          }`}>
                          {cat.count}
                        </span>
                      )}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6 md:gap-8">
              {products.map((product) => (
                ...
                >
                  ...
                  {/* Content Section */}
                  <div className="flex justify-center  p-2 lg:p-3">
                    <h4 className="text-[16px] font-bold text-slate-900 leading-snug group-hover:text-teal-600 transition-colors line-clamp-2">
                      {product.title}
                    </h4>
